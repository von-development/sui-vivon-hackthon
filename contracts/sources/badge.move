module guard::badge {
    ...
    public struct WinnerBadge has key, store {
        id: UID,
        pool_id: sui::object::ID,         // Which bounty pool was won
    ...
    public fun mint_badge(
        ...
        let badge_uid = sui::object::new(ctx);
        let badge_id = sui::object::uid_to_inner(&badge_uid);
        
        // Create dynamic badge content
        let name = std::string::utf8(b"JailbreakGuard Winner Badge");
    ...
    /// Get badge pool ID
    public fun badge_pool_id(badge: &WinnerBadge): sui::object::ID {
        badge.pool_id
    }
    ...
    fun create_description(reward_amount: u64): String {
        let mut base_description = std::string::utf8(b"Congratulations! You successfully demonstrated a jailbreak vulnerability and earned ");
        let sui_amount = format_sui_amount(reward_amount);
        let closing = std::string::utf8(b" SUI from the bounty pool. Your contribution helps improve AI safety for everyone.");
        
        std::string::append(&mut base_description, sui_amount);
        std::string::append(&mut base_description, closing);
        
        base_description
    }

    /// Create image URL (could be generated dynamically or point to static asset)
    fun create_image_url(_pool_id: sui::object::ID, _hunter: address): String {
        // For MVP, use a static badge image
        // In production, this could generate unique images based on pool_id and hunter
        std::string::utf8(b"https://raw.githubusercontent.com/von-development/assets-for-sui/main/winner-badge.png")
    }
    ...
    fun format_sui_amount(amount_mist: u64): String {
        ...
        if (remainder == 0) {
            // Whole SUI amount
            std::string::utf8(b"1") // Simplified - in production would properly format numbers
        } else {
            // Decimal SUI amount
            std::string::utf8(b"0.1") // Simplified - in production would properly format decimals
        }
    }
